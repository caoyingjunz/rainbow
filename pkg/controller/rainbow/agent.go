...
func (s *AgentController) Search(ctx context.Context, date []byte) error {
	...
	case 3:
		result, err = s.SearchImageInfo(ctx, reqMeta.ImageInfoRequest)
	default:
...
func (s *AgentController) SearchImageInfo(ctx context.Context, req types.RemoteImageInfoRequest) ([]byte, error) {
	switch req.Hub {
	case "dockerhub":
